...
class PolicyWithValue(object):
    ...
    def step(self, observation, **extra_feed):
        ...
        **extra_feed    additional data such as state or mask (names of the arguments should match the ones in constructor, see __init__)
        ...
        a, v, state, neglogp, latents = self._evaluate([self.action, self.vf, self.state, self.neglogp, self.train_intermediate_feature], observation, **extra_feed)
        if state.size == 0:
            state = None
        return a, v, state, neglogp, latents
