...
class PolicyWithValue(object):
    ...
    def step_evaluate(self, observation, **extra_feed):
        """
        Compute next action(s) given the observation(s)
        ...
        **extra_feed    additional data such as state or mask (names of the arguments should match the ones in constructor, see __init__)
        ...
        a, v, state, neglogp, latent = self._evaluate([self.action, self.vf, self.state, self.neglogp, self.train_intermediate_feature], observation, **extra_feed)
        if state.size == 0:
            state = None
        return a, v, state, neglogp, latent
...
def build_policy(env, policy_network, value_network=None,  normalize_observations=False, estimate_q=False, num_levels=0, **policy_kwargs):
